...
function BlockchainProvider({ children }) {
  ...
  const [votingAppContract, setVotingAppContract] = useState({});
  ...
  async function connectToBlockchain() {
    ...
    if (networkData) {
      ...
      const _votingAppContract = new web3.eth.Contract(
        VotingApp.abi,
        networkData.address
      );
      // console.log(_votingAppContract);
      setVotingAppContract(_votingAppContract);
    } else {
  ...
  return (
    <BlockchainContext.Provider
      value={{ account, votingAppContract, getErrorMessage }}
    >
